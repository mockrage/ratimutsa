// Plain JavaScript seed script - runs with: NODE_PATH=./node_modules node prisma/seed.js
// No tsx/ts-node needed. Uses @prisma/client and bcryptjs from standalone bundle.
...
async function main() {
  ...
  const regularAdmin = await prisma.user.upsert({
    ...
    update: { password: hashedPassword, role: 'ADMIN', adminRole: 'REGULAR_ADMIN' },
    create: {
  ...
  const seniorAdmin = await prisma.user.upsert({
    ...
    update: { password: hashedPassword, role: 'ADMIN', adminRole: 'SENIOR_ADMIN' },
    create: {
  ...
  var vegetablesCategory = await prisma.category.findUnique({ where: { slug: 'vegetables' } });
  var poultryCategory = await prisma.category.findUnique({ where: { slug: 'poultry' } });
  var livestockCategory = await prisma.category.findUnique({ where: { slug: 'livestock' } });
  var seafoodCategory = await prisma.category.findUnique({ where: { slug: 'seafood' } });

  // Broilers (Poultry)
  if (poultryCategory) {
    var broilers = await prisma.product.upsert({
      where: { slug: 'broilers' },
      update: {},
      create: {
        name: 'Broilers',
    ...
    // Eggs
    var eggs = await prisma.product.upsert({
      where: { slug: 'eggs' },
      update: {},
      create: {
        name: 'Eggs',
  ...
  if (vegetablesCategory) {
    var tomatoes = await prisma.product.upsert({
      where: { slug: 'tomatoes' },
      update: {},
      create: {
        name: 'Tomatoes',
    ...
    // Cabbage
    var cabbage = await prisma.product.upsert({
      where: { slug: 'cabbage' },
      update: {},
      create: {
        name: 'Cabbage',
  ...
  if (livestockCategory) {
    var pigs = await prisma.product.upsert({
      where: { slug: 'pigs' },
      update: {},
      create: {
        name: 'Pigs',
    ...
    // Pork Cuts
    var pork = await prisma.product.upsert({
      where: { slug: 'pork' },
      update: {},
      create: {
        name: 'Pork',
    ...
    // Beef
    var beef = await prisma.product.upsert({
      where: { slug: 'beef' },
      update: {},
      create: {
        name: 'Beef',
  ...
  // Chicken (Poultry)
  if (poultryCategory) {
    var chicken = await prisma.product.upsert({
      where: { slug: 'chicken' },
      update: {},
      create: {
        name: 'Chicken',
  ...
  // Seafood category (already created above)
  // Fish (Seafood)
  if (seafoodCategory) {
    var fish = await prisma.product.upsert({
      where: { slug: 'fish' },
      update: {},
      create: {
        name: 'Fresh Fish',
  ...
  // Peppers (Vegetables)
  if (vegetablesCategory) {
    var peppers = await prisma.product.upsert({
      where: { slug: 'peppers' },
      update: {},
      create: {
        name: 'Peppers',
    ...
    // Cucumbers
    var cucumbers = await prisma.product.upsert({
